...
Export normally with the standard Godot 4.7 templates: downloaded patches load automatically before the game starts and remain active on later launches. Existing players need a fresh executable once to receive this fix; no custom template or `disable_path_overrides` option is needed. Engine upgrades still require a new executable.
