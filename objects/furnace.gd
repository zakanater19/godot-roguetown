...
var _fuel_type:    String = "" # "Coal" or "Log" — matches item_type from the .tres
...
func _input_event(_viewport: Viewport, event: InputEvent, _shape_idx: int) -> void:
	...
	# Range check — must be adjacent tile
	...
	if event is InputEventMouseButton and event.button_index == MOUSE_BUTTON_LEFT and event.pressed:
		...
		if held != null:
			...
			if held.get("is_fuel") == true:
				if _coal_count >= 1:
					...
					if multiplayer.is_server():
						World.rpc_request_furnace_action(get_path(), "insert_fuel:" + itype, player.active_hand)
					else:
						World.rpc_request_furnace_action.rpc_id(1, get_path(), "insert_fuel:" + itype, player.active_hand)
			elif held.get("is_smeltable_ore") == true:
				if _ironore_count >= 1:
					player._show_inspect_text("already has ore", "")
				else:
					if multiplayer.is_server():
						World.rpc_request_furnace_action(get_path(), "insert_ore", player.active_hand)
					else:
						World.rpc_request_furnace_action.rpc_id(1, get_path(), "insert_ore", player.active_hand)
			else:
			...
			if _coal_count >= 1 and _ironore_count >= 1:
				if multiplayer.is_server():
					World.rpc_request_furnace_action(get_path(), "start_smelt", player.active_hand)
				else:
					World.rpc_request_furnace_action.rpc_id(1, get_path(), "start_smelt", player.active_hand)
			else:
				player._show_inspect_text("needs fuel and iron ore to start", "")

	# ── RIGHT CLICK — empty the furnace ─────────────────────────────
		...
		if multiplayer.is_server():
			World.rpc_request_furnace_action(get_path(), "eject", player.active_hand)
		else:
			World.rpc_request_furnace_action.rpc_id(1, get_path(), "eject", player.active_hand)
...
func _perform_action(action: String, player: Node, hand_idx: int, generated_names: Array) -> void:
	if action.begins_with("insert_fuel:"):
		if _coal_count < 1:
			_coal_count += 1
			# The action string carries the item_type, e.g. "insert_fuel:Coal" or "insert_fuel:Log"
...
func _eject_contents(player: Node, generated_names: Array) -> void:
	var drop_offsets := [
		Vector2(-TILE_SIZE, 0), Vector2(TILE_SIZE, 0),
		Vector2(0, TILE_SIZE),  Vector2(-TILE_SIZE, TILE_SIZE)
	]
	var slot := 0
	var name_idx := 0

	if _coal_count > 0:
		# Look up the scene path via ItemRegistry using the stored item_type
...
func _spawn_item(scene: PackedScene, offset: Vector2, _player: Node, node_name: String = "") -> void:
	...
	if node_name != "":
		obj.name = node_name
	obj.position = global_position + offset
