...
# Generic item script — assign the matching ItemData .tres to item_data in the Inspector.
# Replaces all the boilerplate per-item .gd files for simple pickable objects and clothing.
...
# These vars are populated from item_data in _ready() so that external code
# using node.get("item_type"), node.get("force"), etc. continues to work.
