...
func perform_break(drops_data: Array) -> void:
	...
	for data in drops_data:
		...
		if data.type == "pebble": scene = PEBBLE_SCENE
		elif data.type == "coal": scene = load("res://objects/coal.tscn") as PackedScene
		elif data.type == "ironore": scene = load("res://objects/ironore.tscn") as PackedScene
