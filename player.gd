...
# --- CONFIGURATION ---
# TILE_SIZE / GRID_WIDTH / GRID_HEIGHT are owned by the World autoload.
# All tile-size arithmetic here uses World.TILE_SIZE.
...
# ===========================================================================
# Component Wrappers (Routing to backend scripts)
# ===========================================================================

func get_description() -> String:
	return backend.get_description() if backend else character_name

func get_detailed_description() -> String:
	return backend.get_detailed_description() if backend else character_name

func get_inspect_color() -> Color:
	return backend.get_inspect_color() if backend else Color.WHITE

func get_inspect_font_size() -> int:
	return backend.get_inspect_font_size() if backend else 11

func _apply_class_defaults() -> void:
	if backend: backend.apply_class_defaults()
...
func _equip_clothing(item: Node) -> void:
	if backend: backend.equip_clothing(item)

func _equip_clothing_to_slot(item: Node, slot_name: String) -> void:
	if backend: backend.equip_clothing_to_slot(item, slot_name)

func _perform_equip(item: Node, slot_name: String, hand_index: int) -> void:
	if backend: backend.perform_equip(item, slot_name, hand_index)

func _unequip_clothing_from_slot(slot_name: String) -> void:
	if backend: backend.unequip_clothing_from_slot(slot_name)

func _perform_unequip(slot_name: String, new_node_name: String, hand_index: int) -> void:
	if backend: backend.perform_unequip(slot_name, new_node_name, hand_index)

func _inspect_at(world_pos: Vector2) -> void:
	if backend: backend.inspect_at(world_pos)

func _show_inspect_text(text: String, detailed_desc: String) -> void:
	if backend: backend.show_inspect_text(text, detailed_desc)

func _apply_action_cooldown(item: Node, is_attack: bool = false) -> void:
	if backend: backend.apply_action_cooldown(item, is_attack)
...
func get_strength_damage_modifier() -> float:
	return combat.get_strength_damage_modifier() if combat else 0.0

func _get_weapon_damage(item: Node) -> int:
...
func _sync_sleep_state_update(new_state: SleepState) -> void:
	...
	if multiplayer.has_multiplayer_peer():
		if multiplayer.is_server():
			...
			rpc_id(1, "_sync_sleep_state", new_state)
...
func toggle_lying_down() -> void:
	...
	if not is_lying_down:
		...
		if multiplayer.has_multiplayer_peer():
			if multiplayer.is_server():
				rpc("_rpc_sync_lying_down", is_lying_down)
			else:
				rpc_id(1, "_rpc_sync_lying_down", is_lying_down)
	else:
...
func _on_authority_changed() -> void:
	if _is_local_authority():
		...
		if _dead_container:
			_dead_container.visible = dead

# ===========================================================================
# Sync Hands (for late joiners)
# ===========================================================================
	...
	for slot in slots:
		...
		if slot[1] == "waist":
			if item_name != null and item_name != "":
				...
				# Depth sorting: Behind player when facing North (1)
				if facing == 1:
					sprite.z_index = -1
				else:
					sprite.z_index = 4
		...
		if item_name != null and item_name != "" and CLOTHING_TEXTURES.has(item_name):
			...
			if slot[1] == "head" and target_rot == 90.0:
				sprite.position = Vector2(0, -10)
			else:
				sprite.position = cd.offset
...
func show_stats_skills() -> void:
	...
	if prices_shown:
		lines.append("[color=#ffff44]Special Skill: Prices Shown[/color]")
	
	for line in lines:
		Sidebar.add_message(line)
...
func _process(delta: float) -> void:
	...
	if is_local and _sleep_blackout != null:
		if sleep_state == SleepState.AWAKE:
			_sleep_blackout.color.a = 0.0
		elif sleep_state == SleepState.FALLING_ASLEEP:
			_sleep_blackout.color.a = clamp(1.0 - (sleep_timer / 10.0), 0.0, 1.0)
		elif sleep_state == SleepState.ASLEEP:
			_sleep_blackout.color.a = 1.0
		elif sleep_state == SleepState.WAKING_UP:
			_sleep_blackout.color.a = 1.0 if sleep_timer > 2.0 else 0.0
	...
	if is_local:
		if sleep_state == SleepState.AWAKE:
			if _chat_input == null or not _chat_input.has_focus():
				if combat_mode and get_window().has_focus():
	...
	if is_local and _stand_up_timer >= 0.0:
		...
		if buffered_dir != Vector2i.ZERO:
			...
			for _d in range(dot_count):
				dot_str += "."
			if _stand_up_label != null and is_instance_valid(_stand_up_label):
				_stand_up_label.text = dot_str
			if _stand_up_timer >= STAND_UP_DURATION:
				_complete_stand_up()
...
func _unhandled_input(event: InputEvent) -> void:
	...
	if event is InputEventKey and event.keycode == KEY_X and event.pressed and not event.echo:
		...
		if _throw_label != null:
			_throw_label.visible = throwing_mode and hands[active_hand] != null
			
		if multiplayer.has_multiplayer_peer():
			if multiplayer.is_server():
				rpc("_sync_active_hand", active_hand)
			else:
				rpc_id(1, "_sync_active_hand", active_hand)
		return
	...
	if event is InputEventMouseButton and event.button_index == MOUSE_BUTTON_RIGHT and event.pressed:
		...
		if diff.x <= 1 and diff.y <= 1 and target_tile != tile_pos:
			if combat_mode and hands[active_hand] == null:
				...
				if exhausted:
					Sidebar.add_message("[color=#ffaaaa]You are too exhausted to shove![/color]")
				else:
					if stamina < 5.0:
						exhausted = true
						Sidebar.add_message("[color=#ffaaaa]You overexerted yourself![/color]")
					...
					if multiplayer.is_server():
						World.rpc_request_shove(target_tile)
					else:
	...
	if event is InputEventMouseButton and event.button_index == MOUSE_BUTTON_LEFT and event.pressed:
		...
		if hands[active_hand] != null and throwing_mode:
			if exhausted:
				Sidebar.add_message("[color=#ffaaaa]You are too exhausted to throw![/color]")
			else:
				if stamina < 5.0:
					exhausted = true
					Sidebar.add_message("[color=#ffaaaa]You overexerted yourself![/color]")
...
func _update_water_submerge() -> void:
	const FULL_H: int = 32
	const CLIP_H: int = 22
	var on_water := World.tilemap != null and World.tilemap.get_cell_source_id(tile_pos) == 5
	...
	if on_water:
		...
		current_move_duration = (MOVE_TIME * stamina_penalty) * sprint_mult * lying_mult
...
func _on_reconnection_confirmed() -> void:
	...
	if _is_local_authority() and _hud != null:
		_hud.update_clothing_display(equipped)
	
	if dead:
		_die_visuals()
	elif sleep_state != SleepState.AWAKE:
		_set_lying_down_visuals(true)
	
	if _is_local_authority():
		Sidebar.add_message("[color=#aaffaa]You have reconnected to your body![/color]")
