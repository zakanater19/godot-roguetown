...
func calculate_combat_roll(attacker: Node, defender: Node, base_amount: int, is_sword_attack: bool) -> Dictionary:
	...
	if attacker != null and attacker.is_in_group("player") and is_sword_attack:
		if "skills" in attacker: a_skill = attacker.skills.get("sword_fighting", 0)
	...
	var can_defend = true
	if "stamina" in defender and defender.stamina < 3.0: can_defend = false
	if "exhausted" in defender and defender.exhausted: can_defend = false
	if "grabbed_by" in defender and defender.grabbed_by != null and is_instance_valid(defender.grabbed_by): can_defend = false
	if can_defend:
		if d_stance == "parry" and d_has_sword:
			var d_skill = 0
			if "skills" in defender: d_skill = defender.skills.get("sword_fighting", 0)
			...
			if valid_dodge_tiles.is_empty():
				...
				var d_agility = 10
				if "stats" in defender: d_agility = defender.stats.get("agility", 10)
...
func deal_damage_at_tile(tile: Vector2i, z_level: int, amount: int, attacker_id: int = 0, is_sword_attack: bool = false) -> Dictionary:
	...
	for entity in entities:
		...
		results[entity] = roll
		if roll.damage > 0:
			if entity.is_in_group("player"): entity.receive_damage.rpc(roll.damage)
...
func release_grab_for_peer(grabber_peer_id: int, silent: bool = false) -> void:
	...
	if target_node and is_instance_valid(target_node):
		if entry.get("is_player"): target_name = target_node.character_name if "character_name" in target_node else ""
...
func drag_grabbed_entity(grabber_peer_id: int, old_tile: Vector2i) -> void:
	...
	if entry.get("is_player"):
		...
		world.rpc_confirm_move.rpc(entry.get("target_peer_id"), old_tile, false)
	else:
...
func server_try_resist(peer_id: int) -> void:
	...
	if grabbed == null or grabbed.dead: return
	...
	for gp_id in world.grab_map:
		...
		if entry.get("is_player") and entry.get("target_peer_id") == peer_id:
			...
			break
	if grabber == null or grabber_peer_id == -1 or grabbed.exhausted: return
	var total_str = max(float(grabbed.stats.get("strength", 10)) + float(grabber.stats.get("strength", 10)), 1.0)
	var resist_cost = 20.0 * (float(grabber.stats.get("strength", 10)) / total_str)
	var grabber_cost = 20.0 * (float(grabbed.stats.get("strength", 10)) / total_str)
	var break_chance = (float(grabbed.stats.get("strength", 10)) / total_str) * 100.0
...
func handle_rpc_deal_damage_at_tile(sender_id: int, tile: Vector2i, targeted_limb: String) -> void:
	...
	if attacker.body != null and attacker.body.is_arm_broken(attacker.active_hand): return
	if (tile - attacker.tile_pos).abs().x > 1 or (tile - attacker.tile_pos).abs().y > 1: return
	if not world.utils.server_check_action_cooldown(attacker, true): return
	...
	for entity in entities:
		...
		var t_name = ""
		if entity.is_in_group("player"):
			t_name = (entity as Node2D).character_name
...
func handle_rpc_request_grab(sender_id: int, target_path: NodePath, limb: String) -> void:
	...
	if grabber == null or grabber.dead or grabber.hands[grabber.active_hand] != null: return
	if grabber.body != null and grabber.body.is_arm_broken(grabber.active_hand): return
	var now_ms := Time.get_ticks_msec()
	...
	var target_peer = target.get_multiplayer_authority() if is_player else -1
	...
	var t_name = (target as Node2D).character_name if is_player else (target.get("item_type") if target.get("item_type") else target.name.get_slice("@", 0))
	world.rpc_confirm_grab_start.rpc(sender_id, is_player, target_peer, target_path, grabber.character_name, t_name, safe_limb, grabber.active_hand)
...
func handle_rpc_confirm_grab_start(grabber_peer_id: int, is_player: bool, target_peer_id: int, target_path: NodePath, grabber_name: String, target_name: String, limb: String, grab_hand: int) -> void:
	...
	if grabber and target:
		...
		if is_player and target_peer_id != -1:
			...
			if g_player and not g_player.dead and g_player._is_local_authority():
				g_player.grabbed_by = grabber
				g_player._update_grab_ui()
...
func handle_rpc_confirm_grab_released(grabber_peer_id: int, is_player: bool, target_peer_id: int, grabber_name: String, target_name: String, silent: bool) -> void:
	var grabber: Node2D = world.utils.find_player_by_peer(grabber_peer_id) as Node2D
	if grabber and grabber._is_local_authority():
		grabber.grabbed_target = null
		grabber.grab_hand_idx = -1
		grabber._update_grab_ui()
	if is_player and target_peer_id != -1:
		var g_player: Node2D = world.utils.find_player_by_peer(target_peer_id) as Node2D
		if g_player and g_player._is_local_authority():
			g_player.grabbed_by = null
			g_player._update_grab_ui()
...
func handle_rpc_confirm_resist_result(grabber_peer_id: int, grabbed_peer_id: int, broke_free: bool) -> void:
	...
	if broke_free:
		...
		if grabbed and grabbed._is_local_authority():
